...
// this component has changed to governance again
...
const ViewPointCard = ({
  ...
  return (
    ...
    <div
      className="col-6 col-md-3 col-lg-3 viewpoint-sec"
      style={{
        // margin: '10px 10px',
        padding: "0px auto",
      }}
      ...
      <Card className="viewpoint-crd">
        <Link to={`/post/${slug}`}>
          ...
          />
          {
            ...
            post_type === "premium" && (
              <span className="premium-tag premium_category_indicator--viewpoint">
                {post_type}
              </span>
            )
          }
        ...
        <Card.Body className="viewpoint-text">
          <Card.Text className="card-text">
            <Link to={`/post/${slug}`} className="viewpoint-heading">
              {post_title.slice(0, 60)}
            </Link>
